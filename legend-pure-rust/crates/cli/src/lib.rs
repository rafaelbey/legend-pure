...
#![forbid(unsafe_code)]
#![allow(missing_docs)] // internal CLI plumbing; only main_entry is part of the consumer API
...
// The force-link `use` statements for the in-tree runtime extensions
// live on the BIN side (`src/main.rs`), not here. The library is
// extension-agnostic so downstream consumers can pull it in
// alongside their OWN distributed-slice contributions without
// inheriting the in-tree relational stack.
...
/// Library entry point for downstream binaries.
///
/// Parses argv, dispatches to the requested subcommand, prints
/// diagnostics, and exits with the appropriate status code. The
/// in-tree `legend` binary's `fn main()` is a one-line wrapper
/// around this; external CLI binaries (e.g. the worked
/// `examples/mydsl-extension/`) call it the same way after pulling
/// in their own extension crates as link-forced deps.
///
/// Subcommands route through `NativeRegistry::discovered()`,
/// `pipeline::compile()`, `Evaluator::builder().build()`, etc., so
/// any `#[distributed_slice]` extension registered by the calling
/// binary's dep graph is picked up automatically.
pub fn main_entry() {
    let cli = Cli::parse();
