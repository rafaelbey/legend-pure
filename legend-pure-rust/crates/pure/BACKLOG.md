...
| Numeric coercion (`Integer → Float`) | P2 | 🔲 Deferred | Java implicitly widens Integer to Float / Decimal in numeric contexts. Strict mode rejects `compare(1, 2.2)` and `rem(Integer, Float)` because our `bind_type`'s LUB widens T to `Number`, then the strict per-arg check rejects Integer or Float against Number. ~80+ platform errors. |
