...
| `eval` strict arg validation (Java divergence) | P1 | 🚧 In progress | **Reframed**: this is a *strict-mode divergence over Java*, not a parity bug. Reading primary source (`FunctionExpressionProcessor.java:121-265`, `TypeInferenceContext.java:325-495`, `register():467-480`) shows Java itself silently widens for `eval(func:Function<{Integer->X}>, 'wrong')` — the existing-concrete + incoming-concrete branch calls `findBestCommonGenericType(...)` which LUBs covariantly, so `(Integer, String) → Any`. No `TestFunctionTypeInference` test asserts an error for this shape; it's a known-broken corner Java acknowledges in the comment at `register():474-478`. Two prior auth-vs-constraint spikes (commits `c17a06`, reverted `3f1a64`) regressed platform fold-style chains because they inverted Java's order. The new approach: port Java's two-branch dispatch precisely (Step 3d of plan), then layer a strict-mode flag on top (Step 3g) that catches wrong-arg-types via per-arg `is_type_compatible(arg, make_concrete(param))`. Foundation landed Steps 1-3c (`crates/pure/src/inference/`): TDD scaffold, module skeleton, `make_concrete` substitution entry point, scaffold for strict-mode unresolved-generic checks, `TypeInferenceContext` + `RegisterMode` skeleton. Open: 3d (split `infer_function_call` into `FunctionCallProcessor` phases), 3e (`LambdaParamFiller`), 3g (strict-mode flag wiring). Locked by `#[ignore]`'d tests `function_type_one_arg_wrong_type_errors` and `function_type_higher_order_wrong_inner_type_errors` in `crates/pure/tests/integration_tests.rs`, plus the broader `tic_*` set in `crates/pure/tests/inference_context.rs`. |
