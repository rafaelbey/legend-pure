...
| Variance (contravariant `<<-T>>`) | P1 | ✅ Done (min viable) | `Class.type_parameter_variances: Vec<Variance>` (position-aligned with `type_parameters`, `#[serde(default)]`). Two surface forms collapse onto the same flag: m3 metamodel `^TypeParameter{contravariant: true}` (captured by `m3_parser.rs::parse_type_parameter_instance`) and class-level `<-T>` / `<+T>` prefix syntax (parser already accepts; AST → Class wiring uses default Invariant for synthetics). `subtype_view` applies a Nil-→-Any lift for contravariant slots when substituting a class's type-args into super-types — that's the single hot spot that makes contravariance flow through Property's reflective-dispatch chain without touching every consumer. -18 platform strict-mode errors (all 14 `dynamicNew.pure` cleared, plus eval.pure / canReactivateDynamically variance cases). Tests: `crates/pure/tests/variance_tests.rs` (5 cases incl. negative + platform-load wire pin). Docs: `docs/PURE_LANGUAGE_SPEC.md` §4.5. |
| Strict-mode auth/constraint binding distinction | P1 | ✅ Done | Track `Generic(T)` bindings sourced from a structural FunctionType slot (Pure-invariant) separately from those sourced from a top-level Generic-typed arg (Java-style covariant LUB). Strict-mode arg-type check substitutes via `ty_auth` only; constraint-only T's stay `Generic("T")` and `is_type_compatible`'s wildcard arm accepts. `eval(intFunc, 'wrong')` catches; `compare(1, 'a')` / `compare(1, 2.2)` don't false-positive (matches Java's LUB-to-common-supertype semantics for top-level Generic params). Replaced the older excluding-self workaround. -65 platform strict-mode errors. Commit `eaf4ee4cd44`. |
| Let-bound lambda binding chain | P1 | ✅ Done | Three coordinated fixes: (1) Collection LUB preserves type/mult arguments for same-element pairs in `lower::let_expr::infer_let_type`, (2) `type_lub` handles FunctionType-vs-FunctionType (param Nil fallback, return covariant LUB), (3) strict-mode-only bridge in `bind_type_with_mode` for bare-FunctionType arg vs Named-Function-shaped param so let-bound lambdas (whose `var_types[name]` is bare FT after `process_let_function_call` copies `arg_types[1]`) flow into Pass-1 binding. -63 platform strict-mode errors. Commit `f10589b06e6`. Locked by `tic_let_bound_collection_of_lambdas_match`. |
| Function-ref Generic-alias chain resolution | P1 | ✅ Done | Function-ref like `reverse_T_m__T_m_->eval([1,2,3])` lifts to `FunctionType{Generic("T")[Variable("m")]→Generic("T")[Variable("m")]}` carrying the callee's generic names verbatim. Eval's structural binding records T_eval → Generic("T"), V_eval → Generic("T"); arg 1 binds T_eval = Integer; substitute_type now follows Generic→Generic alias chains so V_eval also resolves to Integer. Plus `type_lub` skips Generic-as-LUB-side (treat as identity element so a function-ref's placeholder doesn't widen the caller's binding to Any). -8 platform strict-mode errors. Commit `48d4081c577`. Locked by `tic_function_ref_eval_two_args_binds_through_lift` (un-ignored). |
| Misc strict-mode chain inference (toOne/first/sort/cast/class) | P2 | 🔲 Deferred | ~10 residual strict-mode errors at chain-inference depths: `match.pure:185 $z.genericType.rawType->toOne()`, `PropertyMappingsImplementation:137 $resolved.first.values->first()->concatenate(...)->first()`, `evaluateAndDeactivate:28 .expressionSequence->cast(@InstanceValue)`, `groupBy:39 ->keys().name->sort()`, `class.pure:49 enum.field->class()`. Pattern: a long property/dispatch chain where receiver type or multiplicity propagation drops a binding mid-chain. Per-case targeted fix needed (no shared root cause); all fall under "Generic unification (`Z` propagation)" P2 which Java solves via `TypeInferenceObserver`'s multi-pass. |
