...
| Item | Priority | Status | Notes |
|---|---|---|---|
| Move chunk 0 to compile-time | P2 | ✅ Done 2026-05-20 | `OnceLock`-cached `BootstrapTemplate` in `crates/pure/src/bootstrap.rs`. First `create_bootstrap_chunk` call still parses `m3.pure` (one-shot ~5.5 ms); subsequent calls clone the cached arenas in ~18 µs median (was ~4.5 ms baseline — ~250× speedup). build.rs / proc-macro option was rejected — serialising-then-deserialising is not faster than parsing-then-cloning, adds build-time complexity, and forces a recompile on every `m3.pure` edit. Side-channel `M3_REGISTRATIONS` thread-local removed: `create_bootstrap_chunk` now returns `(ModelChunk, Vec<M3Registration>)` and `register_m3_packages` takes the registrations as an explicit parameter. |
...
| Constraint evaluation (compile-time validation) | — | ✅ Shipped 2026-05-18 | Inline in `lower_constraints` (`crates/pure/src/pipeline.rs`): after each constraint body / message is lowered, the type is inferred on the spot via `infer_typeexpr_from_valuespec` + `infer_multiplicity_from_valuespec` (both already work mid-Pass-2b once signatures are hydrated) and checked against the slot's required shape — `Boolean[1]` for `Constraint.function`, `String[1]` for `Constraint.message`. Mismatch surfaces `CompilationErrorKind::ConstraintBodyTypeMismatch` at the constraint's source span. Validator-style post-hoc check was the prior design; switched to inline at the source-of-truth (constraint construction time) per user feedback, avoiding a `pass_infer` extension + cross-chunk pass. Same `$this` + `type_variable_parameters` scope as QP-body lowering. LSP `diagnostics.rs` carries the kind label `"constraintBodyTypeMismatch"`. Locked by `crates/pure/tests/negative_tests.rs::neg_class_constraint_body_not_boolean_errors`, `neg_class_constraint_message_not_string_errors`, and `pos_class_constraint_with_valid_signatures_compiles`. Platform 0 errors; surveyor 398/0/0; full workspace clean. |
