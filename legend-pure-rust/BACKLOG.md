...
| Milestoning — Phase F (Pure→Protocol patch) | Done ✅ | `legend parse --compile` now runs the compile pipeline over the parsed sources and patches each `Class` / `Association` protocol element with `originalMilestonedProperties` populated from the compiled model. Conversion lives in `crates/cli/src/commands/parse_compiled.rs`, which renders compiled `pure::Class::Property` values back as protocol `Property` (full path, multiplicity, stereotypes, tagged values, aggregation, source info). Without `--compile` the output remains pure AST→Protocol shape. Test: `crates/cli/tests/cli_integration.rs::test_parse_compile_flag_emits_original_milestoned_properties`. **Out of scope here:** rewriting `properties` / `qualified_properties` to the post-synthesis shape, and converting lowered expression bodies — those need a full Pure→Protocol path; the patch step is intentionally minimal and focused on what the milestoning surface actually needs. Tracking those follow-ups separately under the broader Pure→Protocol entry. |
