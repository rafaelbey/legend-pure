...
| Item | Priority | Notes |
|------|----------|-------|
| Full generic unification (`Z` propagation) | P2 | Audit-only audit completed (Item 3, commit pending). Platform precision sweep ceiling holds at **0** body-Any cases; per-expression drift histogram baseline is **5931** surviving Generic+Variable markers across 563 functions (`crates/core-platform-pure/tests/inference_drift_histogram.rs`, ceiling 6500). The dormant `TypeInferenceContext` skeleton (`crates/pure/src/inference/context.rs`) is documented but **deliberately not wired** — both prior structural spikes (`c17a06`, reverted `3f1a64`) regressed platform fold-style chains. **Do not enable without a concrete failing test driving the change.** Concrete known gap: higher-order eval-arg-mismatch (`function_type_higher_order_wrong_inner_type_errors`, `#[ignore]`'d in `crates/pure/tests/integration_tests.rs`) — auth/constraint tracking propagates one level of FunctionType, not two. P1 follow-up: extend `bind_type_with_mode` to track `inside_structural` depth. |
