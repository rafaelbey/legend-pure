...
| Milestoning — Phase F (Pure→Protocol) | Done ✅ | `legend parse --compile` runs the compile pipeline and *replaces* each `Class` / `Association` protocol element with the compiled (post-synthesis) view: `properties` now carries the synthesised `addressAllVersions` edge-points + date properties + milestoning slot, `qualifiedProperties` carries the `address(td)` / `addressAllVersionsInRange(start, end)` signatures, `originalMilestonedProperties` carries the moved-aside originals, and `superTypes` / `stereotypes` / `taggedValues` are emitted from the compiled `ElementId`-backed references. Constraints + property default-values + QP bodies are converted from lowered `ValueSpec` to protocol `ValueSpecification` for the common variants: literals (incl. `%latest`), variables, function calls, property/QP calls, lambdas, collections, enum values, type/element refs, `PathLiteral` → `classInstance("path", …)`, `RelationLiteral` → `classInstance("relationType", { columns: [...] })`, `ColSpecLiteral` / `ColSpecArrayLiteral` → `classInstance("colSpec"\|"colSpecArray"\|"funcColSpec…"\|"aggColSpec…", …)`, and **`MultiplicityReference` → `packageableElementPtr` for the four well-known instances (`PureOne` / `ZeroOne` / `OneMany` / `ZeroMany`) or `classInstance("multiplicity", { lowerBound, upperBound? })` / `(... multiplicityParameter)` for arbitrary ranges and variables**. The only remaining `@unsupported:<kind>` placeholder is for the non-`Named` `TypeReference` shapes (`FunctionType`, `Relation`, `AlgebraUnion` structural type literals), which don't appear in milestoning bodies. Code: `crates/cli/src/commands/parse_compiled.rs` + shared multiplicity helper in `crates/protocol/src/v1/convert.rs::{multiplicity_arg_to_value_spec, protocol_multiplicity_to_value_spec}`. Tests: `crates/cli/tests/cli_integration.rs` covers the synthesised `addressAllVersions` + `addressAllVersionsInRange` shape, the multiplicity wire form, and regression guards confirming none of the now-converted variants fall back to the placeholder. |
