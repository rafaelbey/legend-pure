...
| Milestoning — Phase B (runtime + propagation) | P1 | Open follow-up. Three pieces required for runnable milestoned classes: (1) implement `getAll(Class<T>, Date[1])`, `getAll(Class<T>, Date, Date)`, `getAllVersionsInRange(Class<T>, Date, Date)` natives — `crates/runtime/tests/pct_coverage_audit.rs:60-79` records two of the three as missing today. (2) Populate the synthesized qualified-property bodies (filter-by-date lambdas — empty in Phase A). (3) Compile-time date-context propagation pass: Java's `MilestoningDatesPropagationFunctions` parity, rewriting `$x.address` → `$x.address($contextDate)` based on enclosing lambda / function date scope. Plus three expression-level validators (`%latest` only-in-milestoning, missing-date-arg, `%latest`-not-in-AllVersionsInRange) that depend on the propagation pass to be meaningful. |
