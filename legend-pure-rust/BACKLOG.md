...
| H2 backend for the relational native bodies | P2 | Today only `DatabaseType.DuckDB` routes through to a real DB; every other enum member returns `Relational native <fn>: DatabaseType.<m> is not supported yet`. Java production code expects H2 as the *default* test backend. **Implementation path**: H2 ships a PostgreSQL-wire-compatible mode (`PG_DEFAULT_NULL_ORDERING=high;MODE=PostgreSQL`); spin H2 as a sidecar (in-process Java sub-process or testcontainers) and talk to it via the `postgres` / `tokio-postgres` Rust crate. Two flavours to decide between: (1) **Test-only**: `legend-pure-runtime-java-extension-shared-store-relational/.../TestDatabaseConnect.java` already starts an embedded H2 on `localhost:9092`; have the Rust side opportunistically connect there if it's running, and skip the test otherwise. Lowest friction; tests are end-to-end against a real H2. (2) **Production**: bundle H2's jar (or a pre-built native binary via GraalVM) and start/stop it in `H2State::new` analogous to `DuckDBState`. Adds a JVM dep but no external service. Either way: drop a new `H2State` next to `DuckDBState`, register it under `meta::relational::runtime::DatabaseType.H2` in `dispatch::require_*`, and split the connection-routing match so each backend picks its own state type from `ExtensionStateStore`. The DuckDB → Pure cell-mapping table in `resultset::cell_to_value` already handles every JDBC type H2 produces; the new `H2State::cell_to_value` is a thin wrapper around `tokio_postgres::types::Type` → `Value`. Slip note: H2's PG-compat is incomplete (no `RETURNING`, limited array support); H2-specific assertions in ported tests may need PG-compat-aware SQL. |
