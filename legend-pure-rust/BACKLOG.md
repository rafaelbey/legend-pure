...
| Path-property cloned-inverse propagation | P2 | `^$src(parent.children=[], parent.name='X')` correctly clones the nested object now (testDeepCopyToOne / testDeepCopyWithAssociation1 pass) but the clone's own carried-over association-side properties don't fire their inverses. `testDeepCopyWithAssociation2` fails on `assertSameElements([$tech, $tech1, $p, $tech2], $firmX.organizations)` because the cloned `$p` carries `firm = $firmX` from `$tech` but isn't appended to `$firmX.organizations`. Naively running `populate_association_inverses` on the clone with its full carried property set regresses testDeepCopyToOne — duplicates appear in inverse lists because the source's own inverse-side relationships weren't filtered out. Need: a way to populate inverses for *new* clone references without re-running ones that already point through the source. Likely path: track which association ends were "carried" vs. "newly assigned" and only fire inverse for the latter. |
...
| Map interior mutability | P2 | `Value::Map(Box<im_rc::HashMap<ValueKey, Value>>)` is a value type — `$m->getIfAbsentPutWithKey(3, lambda)` computes the new entry but doesn't persist it back on `$m`, so `$m->get(3)` after the call still returns empty. Two failing platform tests (`testGetIfAbsentPutWithKey`, `testGetMapStats`) depend on Java Pure's interior-mutable Map semantics. **Approach A (minimal change to call sites)**: switch to `Value::Map(Rc<RefCell<im_rc::HashMap<…>>>)` — every read sites `borrow()`, every mutation `borrow_mut()`. ~20 call sites in `runtime/src/native/{collection,meta}.rs` to update. PartialEq comparison stays cheap (compare RefCell contents). Caveat: structural equality of two Maps now needs to tolerate the wrapper. **Approach B (heap-backed Map)**: store map entries on the runtime heap as a hidden Object. Operations mutate the heap. Cleaner alignment with Java Pure where Maps are heap objects but invasive. **Approach C (variable-rebinding for the specific natives)**: detect when arg[0] is `ExprKind::Variable("m")` in `getIfAbsentPutWithKey` (and similar mutators), evaluate the side-effect, then re-bind the variable in the caller's scope to the updated map. Requires the native to inspect args before evaluation and mutate `ctx.context_mut()`. Hacky but doesn't change `Value::Map` shape. Recommendation: Approach A — the smallest blast radius for full interior-mutable semantics. Worth measuring `Rc<RefCell<…>>` overhead on hot-path map operations before committing. |
| Map-Element wrapper for `class()` chain | P2 | `^LA_List<String>(values=…)` correctly carries `__typeArguments` on the heap object now, but `$l1->class()` (which Pure expands to `$any->genericType()->genericTypeClass()->cast(@Class<T>)`) collapses back to a bare `Value::Element(LA_List)` — the parametric binding is lost. `testNewGenericFunc` then fails on `new($l1->class(), '')`'s second/third asserts because the constructed `$l2`'s `genericType().typeArguments` is empty. **Path forward**: change `genericTypeClass` so when its input GenericType has non-empty `typeArguments`, it returns a `Value::Object` wrapping a heap "ClassReference" (rawType + typeArguments) instead of the bare `Value::Element`. `Cast` already passes Value::Object through. `New` then needs a third detection branch — when `args[0]` is a heap Object whose classifier is the `Class` metatype, unwrap rawType to get the class element and seed `__typeArguments` from the wrapper's stored bindings. This dovetails with the existing `New` arg-shape detection (Element vs. Collection-of-Elements vs. Collection-of-KeyExpressions); add Object-as-class as a fourth case. |
| GetterOverride property-access hooks | P2 | `dynamicNew(class, kvs, getterToOne, getterToMany, hiddenPayload)` and the 6-arg variant carry user-supplied lambdas the runtime is supposed to consult on every property read whose value isn't directly bound. The platform tests (`testDynamicNewWithClass`, `testDynamicNewWithGenericType`) construct objects without setting `d`/`ds` (class-typed properties) and assert that `$r.d.name` evaluates the `getterOverrideToOne` lambda with `(receiver, propertyRef)` and uses its result. Today the override lambdas are silently dropped; only the `hiddenPayload` slot is partially routed (still requires GetterOverride heap-wrapper materialisation per the M3 spec). Plan: when `dynamicNew` is called with the hook-bearing overload, allocate a `meta::pure::metamodel::type::GetterOverride` heap object carrying the four lambdas + payload, set the constructed instance's `elementOverride` slot to point at it, and intercept `eval_property_access` on instances whose classifier transitively extends `Any` to consult `elementOverride.getterOverrideToOne` / `getterOverrideToMany` for absent properties. The intercept needs the property-multiplicity to choose which lambda to call (single vs many). |
| Reactivate inner-call dispatch + InstanceValue unwrap | P2 | `[xs]->map(p|$p.lastName)->deactivate()->reactivate()` errors today with `"reactivate: simple-name dispatch for SimpleFunctionExpression without 'func' is not implemented"`. A prior attempt added `find_function_by_simple_name` so the SimpleFunctionExpression's `functionName` slot resolves to a Function Element and dispatches via `call_function` — that flipped the dispatch error but the assertion still fails because the `parametersValues` collection contains InstanceValue-wrapped lambdas / Person collections that need their own unwrap pass before `map(persons, lambda)` can consume them. The single-element InstanceValue → Value::Function unwrap landed in an exploratory commit but reverted (because it broke `testPrimitiveExtensionCastReactivate` whose zero-arg lambdas need to be invoked, not passed through). Path forward: in `reactivate_value` for InstanceValue, unwrap the .values collection but distinguish lambdas by parameter arity — zero-arg lambdas are "deactivated expressions to invoke", parametrised lambdas are "first-class function values to pass through". Plus: the parametersValues collection needs an additional layer of InstanceValue unwrap because the persons themselves were deactivated as InstanceValue { values: [p1, p2, p3] }. |
| Map FunctionType into GenericType.typeArguments for lambdas | P2 | `testEvaluateAndDeactivate` does `{a:String[1], b:Integer[1] | true}->genericType().typeArguments->at(0).rawType->toOne()->cast(@FunctionType).parameters` — the lambda's GenericType is expected to have a typeArguments[0] that is a FunctionType heap wrapper carrying the parameter list. `GenericTypeOf` on a `Value::Function(LambdaClosure)` currently doesn't populate `typeArguments`. Path forward: when the value being inspected is a Function whose closure has parameters, allocate a `meta::pure::metamodel::type::FunctionType` heap wrapper with `parameters` (a list of VariableExpression objects per param name+type) and `returnType`/`returnMultiplicity`, then use `[wrapper]` as the GenericType's `typeArguments`. Same shape as Java Pure's metamodel produces from the m3.pure FunctionType class. |
| addColumns + RelationType heap wrappers | P3 | `addColumns(relType, colSpecArray)` is the entry point for column-extending a RelationType — `testAddColumns` asserts the resulting columns serialize as `'x:String[0..1],ab:String[1..1],z:Integer[0..1]'`. Today RelationType is `TypeExpr::Relation(RelationId)` at the compiler level but has no runtime heap representation; `addColumns` isn't registered. Path forward: define a `RelationType` heap wrapper with a `columns` property (each column is a heap object with `name`, `classifierGenericType`, `multiplicity`), allocate from `@(x:String)` literals via the cast path, implement `addColumns` to merge two column lists. Considerable infrastructure for a single test; lower priority. |
