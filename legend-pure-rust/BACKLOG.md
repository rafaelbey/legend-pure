...
| Non-recursive structural equality | Done ✅ (equality + drop) | `values_equal` and the object-key step are iterative — `Vec<(Value, Value)>` work-stack plus `HashSet<(*const HeapEntry, *const HeapEntry)>` cycle guard (canonicalised pointer pair so `(A, B)` and `(B, A)` collapse). Equality-keyed self-references resolve via "assume equal on cycle" semantics matching Java's `equals`. Removes the `EQ_DEPTH` / `MAX_EQUALITY_DEPTH = 1000` stopgap that previously short-circuited deep inputs to `false`. `Value::Drop` also iterative as of 2026-05-23 — `Vec<Value>` work-stack inside `impl Drop` extracts nested `Collection`s and `UnitInstance`s without recursing on the Rust call stack. Locked by `value::tests::deep_nested_collection_drops_without_overflow` at depth 10 000 (construct + drop; pre-fix overflowed somewhere around 3 000). Equality tests still capped at depth 1 500 because the next recursion source (and now the limiting one) is `#[derive(Clone)] for Value` — every clone of a deeply-nested Collection recurses one frame per level. Open follow-ups in priority order: `Value::Clone` iterative rewrite (the deferral for the equality-depth bump); `pure_to_string` and `render_representation` recurse on `Value::Collection` items (bounded by Pure's user-data depth in practice but the same audit applies); `reactivate_value` recurses on deactivated expression-spec heap objects (AST-shape bounded, lower-priority). |
