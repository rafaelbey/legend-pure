...
| Compiler | 🟡 5 known errors | **5** | ~50+ | 244 files / ~1660 elements (M3 + Store + Mapping + Diagram + Graph + TDS + **Relational** DSL metamodels). The 5 residual errors are all `AmbiguousImport` from the relational embed: 4 calls to `meta::pure::functions::meta::elementToPath` with `Function<Any>[1]` receivers (overload narrowing fails between `Function<Any>` and `PackageableElement` candidates), and 1 call to `meta::relational::metamodel::filterBySchemaName` (single overload yet narrows to 0). Tracked under "Dispatcher narrowing on parameterized supertype overloads" below. |
...
| Dispatcher narrowing on parameterized supertype overloads | P1 | After the `platform_store_relational` embed (Stage 0a, 2026-05-02), `load_platform()` reports 5 `AmbiguousImport` errors. Two patterns: **(a)** 4 calls in `relationalRuntime.pure` — `$this.sqlQueryPostProcessorForPlan->toOne()->elementToPath()` where the receiver is `ConcreteFunctionDefinition<Any>[1]`. The compiler can't pick between `elementToPath(Function<Any>[1])` and `elementToPath(PackageableElement[1])` even though the parameterized overload is strictly more specific. **(b)** 1 call in `functions.pure` — `filterBySchemaName($aliases, $tableAlias.relationalElement->cast(@NamedRelation))` with a single overload yet narrows to 0 (mult/type score mismatch). Path: review `narrow_candidates_by_type` in `crates/pure/src/resolve.rs:740` — confirm scoring prefers `Function<T>` over `PackageableElement` when both match; for case (b), trace why one-overload candidates can narrow to 0. Locked-by tests: `relational_metamodel_resolves_in_platform` (passes — metamodel itself fine); the 5 errors are isolated to function bodies. |
