...
| Compiler | 🟡 4 known errors | **4** | ~50+ | 244 files / ~1660 elements (M3 + Store + Mapping + Diagram + Graph + TDS + **Relational** DSL metamodels). After the `all_candidates` dedup fix in `crates/pure/src/resolve.rs:728`, the residual is 4 `AmbiguousImport` errors — all calls to `meta::pure::functions::meta::elementToPath` in `relationalRuntime.pure` where the receiver is `ConcreteFunctionDefinition<{...}>[1]`. The narrowing pass keeps both `elementToPath(Function<Any>[1])` and `elementToPath(PackageableElement[1])` because the parameterized-type preference isn't expressed in the score / dominance pass. Tracked under "Dispatcher narrowing on parameterized supertype overloads" below. |
...
| Dispatcher narrowing on parameterized supertype overloads | P1 | After the `platform_store_relational` embed (Stage 0a, 2026-05-02), `load_platform()` reports 4 `AmbiguousImport` errors. All 4 are calls in `relationalRuntime.pure` — `$this.sqlQueryPostProcessorForPlan->toOne()->elementToPath()` where the receiver is `ConcreteFunctionDefinition<{...}>[1]`. **Investigation (2026-05-02):** instrumenting `narrow_candidates_by_type` shows the failing call site reaches `narrow` with `arg_types = [None]` — the inner `->toOne()` call hasn't populated its `type_info` at narrow time. All 3 single-arg overloads (`Function<Any>[1]`, `Type[1]`, `PackageableElement[1]`) pass Phase 1 (`is_type_compatible(None, _) → true`), tie in Phase 2, and Phase 3 dominance reads `is_subtype(Function, PackageableElement) = false` — which is *correct* given the m3 data: `m3.pure` line 2104-2111 declares `Function extends Referenceable` directly, NOT `PackageableElement`. (Same for `Type extends Any`.) So Phase 3 has nothing to dominate with. Two viable fix paths: **(a) plumb the inner call's declared return type into `narrow_candidates_by_type` when `type_info` is absent**, so Phase 4's `type_distance` tiebreaker can fire — currently bails because `arg_type_opt` is `None`; **(b) augment the parser's super-type capture so the M3 type lattice has the Function→PackageableElement edge that Java's runtime threads in via `M3CoreInstanceGenerator`** (Java doesn't read m3.pure's `generalizations` literally — it overlays a hand-coded super-type table). **Path (a) is preferred** since it's a strict-improvement to the dispatcher and doesn't bake in Java-specific overlays. Path (b) would also fix the consistency mismatch noted below. **Consistency note:** `is_type_compatible` apparently considers `Measure → PackageableElement` valid (it doesn't filter `Measure` from the `PackageableElement` overload in Phase 1), but `is_subtype` returns false for the same pair. Two different supertype walks. Worth aligning. **Resolved sub-case (2026-05-02):** `filterBySchemaName` was a duplicate-candidate bug (same ElementId arrived via two import scopes); fixed at `resolve.rs:728` with insertion-order-preserving dedup before narrowing. Locked-by tests: `relational_metamodel_resolves_in_platform` (passes — metamodel itself fine); the 4 errors are isolated to `relationalRuntime.pure` function bodies. **User-source cleanliness tests** (`compile_pair_first_compiles_clean`, `compile_known_property_on_generic_type_compiles_clean`, `compile_let_bound_untyped_lambda_emits_inference_error`) were updated to filter platform residuals by `source_info.source == "<test>"`. |
