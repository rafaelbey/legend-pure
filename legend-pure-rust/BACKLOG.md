...
| H2 backend follow-ups | P3 | The H2 backend ships in `crates/store-relational-runtime` (auto-spawned Java sub-process in PG-wire mode + sync `postgres` client, `RelationalBackend` trait dispatching DuckDB / H2 per native, `[extension.relational.h2]` classpath section + env-var overrides). Three known follow-ups: **(a) Canonical Pure `<<test.Test>>` H2 functions** — the platform root surveyor (`eval_surveyor_root_strict_pass`) demands every shipped test passes; H2 tests in `main/resources` would fail without `LEGEND_PURE_H2_JAR`. Need a "skip when external dep unavailable" surveyor mechanism before adding H2 tests there. **(b) NUMERIC / DATE / TIMESTAMP cell mapping** — `cell_to_value_pg` currently stringifies these. Pull in `postgres` features `with-chrono-0_4` (or thread `time` 0.3 → jiff) + a decimal feature, mirror the DuckDB type table. **(c) Linux `PR_SET_PDEATHSIG` polish** — when a `cargo test` parent is SIGKILL'd, the spawned H2 child becomes a zombie. `pre_exec(PR_SET_PDEATHSIG, SIGTERM)` would fix it, but requires unsafe — the crate has `#![forbid(unsafe_code)]`. Either lift the forbid for one isolated module, or accept the limitation (documented; `pkill -f h2.tools.Server` is the manual recovery). **(d) CLI `legend test` classpath integration** — the test command (`crates/cli/src/commands/test.rs`) still falls back to `load_platform()` instead of routing through `resolve_classpath`. Once that lands, call `legend_pure_store_relational_runtime::set_extension_configs(resolved.extension_configs)` so classpath-supplied `[extension.relational.h2]` configs reach the runtime. |
