...
| Move bootstrap chunk 0 to compile-time | P2 | Done ✅ 2026-05-20 — `OnceLock`-cached template in `crates/pure/src/bootstrap.rs`. First call still parses `m3.pure` (~5.5 ms one-shot); subsequent calls clone the cached arenas (~18 µs median per `bootstrap_chunk_bench` example, ~250× speedup vs the 4.5 ms baseline). Side-channel `M3_REGISTRATIONS` thread-local removed; `create_bootstrap_chunk` now returns `(ModelChunk, Vec<M3Registration>)` and `register_m3_packages` takes the registrations as an explicit parameter. The build.rs / proc-macro approach was rejected in favour of the lazy cache: parsing-then-cloning beats serialising-then-deserialising, the lexer/parser stays the source of truth for `m3.pure`, and DX (no codegen step) is preserved. Bench example at `crates/pure/examples/bootstrap_chunk_bench.rs`. |
...
| Non-recursive structural equality | Done ✅ (equality + drop + clone + toString + toRepresentation) | `values_equal` and the object-key step are iterative — `Vec<(Value, Value)>` work-stack plus `HashSet<(*const HeapEntry, *const HeapEntry)>` cycle guard (canonicalised pointer pair so `(A, B)` and `(B, A)` collapse). Equality-keyed self-references resolve via "assume equal on cycle" semantics matching Java's `equals`. Removes the `EQ_DEPTH` / `MAX_EQUALITY_DEPTH = 1000` stopgap that previously short-circuited deep inputs to `false`. `Value::Drop` iterative as of 2026-05-23 — `Vec<Value>` work-stack inside `impl Drop` extracts nested `Collection`s and `UnitInstance`s without recursing on the Rust call stack. `Value::Clone` iterative as of 2026-05-23 — `#[derive(Clone)]` was removed; the manual `impl Clone` uses an explicit-frame DFS (`ops` stack of `Visit`/`Assemble` markers + `built` stack of cloned children) to deep-clone `Collection`/`UnitInstance` bottom-up. The recursion source was `im_rc::Vector::Clone`'s inline-storage path: vectors smaller than `CHUNK_SIZE = 64` (which every level in the deep-nested test fixture is, since each holds 1 element) clone element-by-value via the chunk, so derived `Value::Clone` recursed one frame per level. `render_representation` (`crates/runtime/src/native/meta.rs`, powers `toRepresentation`) and `pure_to_string` (`crates/runtime/src/native/string.rs`, powers `toString`) both ported to the same explicit-frame DFS as of 2026-05-23 — the recursive-`Collection`/`UnitInstance` shape is queued through `Op::Visit`/`Op::AssembleCollection`/`Op::AssembleUnitInstance` markers and a `built: Vec<String>` stack. `pure_to_string`'s `Value::Object` → user-defined `toString()` QP dispatch is **signature-gated** and **single-lookup**: it reuses the same `find_qp_with_generalization` walk the evaluator uses for dispatch (now `pub` in `crates/runtime/src/eval.rs`, returns a `FoundQp` carrying parameters / body / type-var names / return type / multiplicity), checks the declared return is `String[1]`, and then invokes the *already-found* QP through a new `EvalContextTrait::invoke_qualified_property_found(receiver, &FoundQp, args)` method — no second hierarchy walk by name. The existing `invoke_qualified_property(receiver, name, args)` is retained as a convenience that does the lookup then delegates to `invoke_qualified_property_found`. Any non-`String[1]` declared shape (or absent QP) → fall back to `Anonymous_<id>` without running the body; malformed `toString` QPs are never executed. A defensive post-invoke check still falls back if the body returned a non-`String` despite the declared sig. Locks: `value::tests::deep_nested_collection_drops_without_overflow` and `…_clones_without_overflow` at depth 10 000; `native::equality::tests::deeply_nested_collections_compare_*` at depth 3 000 (lower than no-overflow tests because the equality loop is `O(depth²)` — depth 10 000 = ~100 M ops = 30+ s in debug builds); `native::meta::render_tests::render_representation_deep_collection_no_overflow` at depth 10 000; `native::string::tests::pure_to_string_deep_collection_no_overflow` at depth 10 000. `reactivate_value` (in `crates/runtime/src/native/meta.rs`) also ported to the same explicit-frame DFS as of 2026-05-23: a `ReactivateOp` work-stack queues `Visit(Value)` frames for each spec-tree node, with `AssembleInstanceValue { len }` and `AssembleFunctionExpression { len, obj_id }` markers handling the recursive variants (`InstanceValue.values` → flatten via `Value::from_vec`; `SimpleFunctionExpression.parametersValues` → reactivate then `ctx.call_function`). `VariableExpression` is terminal and goes straight to `built`; pass-through (non-Object, unknown classifier) and the 0-arg lambda thunk eval are inline in the visit dispatch. Callable resolution for `FunctionExpression` (preferring `func`, falling back to `functionName`+arity simple-name lookup) lives at assemble-time in `resolve_function_expression_callable` — preserves the recursive version's "reactivate params first, then resolve+invoke" ordering when a param's 0-arg lambda side effect should fire before any resolution error. The three former helpers (`reactivate_instance_value`, `reactivate_variable_expression`, `reactivate_function_expression`) are gone; their dispatch logic lives in `visit_one_spec` inline. With this all five recursion sources (`values_equal`, `Value::Drop`, `Value::Clone`, `render_representation`, `pure_to_string`, `reactivate_value`) are non-recursive across the runtime. |
...
| Milestoning — Phase A (declare-side) | Done ✅ | Classes carrying `<<temporal.businesstemporal>>` / `<<temporal.processingtemporal>>` / `<<temporal.bitemporal>>` now synthesize the date properties (`businessDate` / `processingDate`), the `milestoning: {Business,Processing,BiTemporal}DateMilestoning[0..1]` slot, and — for each declared property whose target is milestoned — the edge-point `pAllVersions: T[*]` + qualified-property signatures (`p(td: Date[1])`, `pAllVersionsInRange(start, end)`, optionally `p()`, optionally `p(pd, bd)` for bitemporal targets). Original properties move to `Class.original_milestoned_properties` / `Association.original_milestoned_properties`. Four declare-side validators wired into `hydrate_element_signature` (at-most-one temporal stereotype / reserved property names / hierarchy consistency) plus an in-synthesis edge-point name collision check (A4.4). Synthesis pass runs between Pass 2a and Pass 2b in `crate::pipeline` — uses resolved property `type_expr`s to detect milestoned targets. Code: `crates/pure/src/milestoning/{mod,synthesis,validate}.rs`, four new `CompilationErrorKind` variants. Tests: `crates/pure/tests/milestoning_{class_synthesis,property_rewrite,validators}.rs` (15 tests). Round-trip lock: the `original_milestoned_properties` slot serialises via the existing postcard `.purem` path (Class + Association schema fingerprints updated, slice/merge walker visits the new slot). Protocol JSON emission landed under Phase F (see below). Java parity: `MilestoningClassProcessor` + `MilestoningPropertyProcessor` + `MilestoningClassValidator`. Plan file: `~/.claude/plans/we-need-now-to-happy-raven.md`. |
| Milestoning — Phase F (Pure→Protocol) | Done ✅ | `legend parse --compile` runs the compile pipeline and *replaces* each `Class` / `Association` / `Function` protocol element with the compiled (post-synthesis, post-inference) view. **Functions**: resolved parameter and return types, lowered body (with operator names like `plus` / `equal` instead of AST's syntactic forms), resolved stereotype / tagged-value references. Compile-failed bodies (e.g. references to symbols not loaded at parse time) keep the AST output unchanged so `--compile` stays graceful. **Classes/Associations**: `properties` carries the synthesised `addressAllVersions` edge-points + date properties + milestoning slot, `qualifiedProperties` carries the `address(td)` / `addressAllVersionsInRange(start, end)` signatures, `originalMilestonedProperties` carries the moved-aside originals, and `superTypes` / `stereotypes` / `taggedValues` are emitted from the compiled `ElementId`-backed references. Constraints + property default-values + QP bodies are converted from lowered `ValueSpec` to protocol `ValueSpecification` for every expression variant: literals (incl. `%latest`), variables, function calls, property/QP calls, lambdas, collections, enum values, packageable-element refs, `PathLiteral` → `classInstance("path", …)`, `RelationLiteral` → `classInstance("relationType", { columns: [...] })`, `ColSpecLiteral` / `ColSpecArrayLiteral` → `classInstance("colSpec"\|"colSpecArray"\|"funcColSpec…"\|"aggColSpec…", …)`, `MultiplicityReference` → `packageableElementPtr` (well-known) or `classInstance("multiplicity", …)` (arbitrary range / variable), and **`TypeReference` non-`Named` shapes** (`FunctionType` → `classInstance("functionType", { parameters, returnType, returnMultiplicity })`, `Relation` → `classInstance("relationType", …)`, `Generic` → `classInstance("genericType", { typeParameter })`, `AlgebraUnion` → `classInstance("algebraUnion", { left, right })`). Only `TypeExpr::Unresolved` (a true compile-time hole, shouldn't appear in clean output) emits a sentinel `Var { name: "@unresolved" }`. Code: `crates/cli/src/commands/parse_compiled.rs::{value_spec_to_protocol, type_reference_to_protocol}` + shared multiplicity helpers in `crates/protocol/src/v1/convert.rs::{multiplicity_arg_to_value_spec, protocol_multiplicity_to_value_spec}`. Tests: `crates/cli/tests/cli_integration.rs` covers milestoning synthesis, multiplicity wire form, and a broad guard (`test_parse_compile_emits_no_unsupported_placeholders`) that exercises path / colspec / relation / multiplicity / type-ref literals in one fixture and asserts no `@unsupported:` placeholder remains anywhere in the output. |
...
| Milestoning — Phase B-3 (date propagation, first slice) | Done ✅ | Pass 2.5b — milestoning date-context propagation — runs in `finalize_model` between Pass 2.5 (inference) and Pass 3 (validation). Rewrites implicit milestoned-target property accesses (`$x.address`) into qualified-property calls (`$x.address($d)`) by detecting the in-scope date from three Java `MilestoningDateSourceType` variants: **VariableExpression** (scope-stack lookup), **getAll(Class, $date)** (call result carries the date downstream), and **filter/map/subType lambdas** (lambda parameter inherits source's milestoning context). Plus the B-4.2 **missing-date validator** — when propagation can't find a date for a confirmed milestoned-target access, a `MilestoningMissingDateContext` error fires with a hint pointing at the explicit-date form. New module `crates/pure/src/milestoning/{dates,propagation}.rs`; wired into `crates/pure/src/pipeline.rs::finalize_model`. New `CompilationErrorKind::MilestoningMissingDateContext`. 3 integration tests in `crates/runtime/tests/milestoning_propagation_smoke.rs` cover the structural rewrite, the runtime-equivalence to the explicit form, and the missing-date error. |
| Milestoning — Phase B-3 (variants 2 + 4) | Done ✅ | Variant 2 — **explicit-QP-propagates-its-dates** — landed in `detect_dates_from_qp_call`: when a QP call carries explicit Date args (`$x.address(%2024-01-01)`), the dates are wrapped in `MilestoningDates::from_source` using the QP's target stereotype and flow downstream to nested milestoned access on the result. Variant 4 — **AutoMap on collection receivers** — verified to already work: `resolved_class_of` returns the element class regardless of receiver multiplicity, so `$customers.address` on `Customer[*]` rewrites the same way as on `Customer[1]`. Both structural and end-to-end tests in `crates/runtime/tests/milestoning_propagation_smoke.rs` cover the two patterns. Remaining 8th variant (`MilestonedGetAll` + `Bitemporal-narrowing` etc.) is implicit in the implementation. |
| Milestoning — Phase B-4 (expression validators) | Done ✅ | All three expression-level validators ship. B-4.2 (missing-date) landed alongside B-3. B-4.1 (`%latest` outside milestoning context) and B-4.3 (`%latest` forbidden in `AllVersionsInRange`) ship in `crates/pure/src/milestoning/validate.rs::validate_latest_usage`, wired in `pipeline.rs::finalize_model` as Pass 2.5c after propagation. |
| Milestoning — Phase D (`%latest` sentinel) | Done ✅ | `%latest` now parses (lexer recognises `%latest` as a `DateLiteral`), lowers to a new `DateValue::Latest` IR variant, and evaluates to a new `Value::Latest` runtime variant. In `compare_values` ordering it ranks greater than every concrete `Date`; in milestoning dispatch (`instance_property_matches_date`) it acts as a wildcard so `getAll(Class, %latest)`, `Class.all(%latest)`, and `$x.address(%latest)` all return every version. Touches: lexer, parser, AST→IR lowerer, `types.rs` (`DateValue::Latest` variant + schema-fingerprint bump), `infer.rs` / `resolve.rs` (`%latest` types as abstract `Date`), runtime `value.rs` (`Value::Latest`), `display.rs` / `meta.rs` / `string.rs` / `comparison.rs` / `lang.rs` (downstream Value handling), and the new B-4.1 + B-4.3 validators. 4 integration tests in `crates/runtime/tests/milestoning_latest_sentinel.rs`. |
| Milestoning — Phase C (grammar shortcuts) | Done ✅ | `Class.all($d)` / `Class.all($pd, $bd)` / `Class.allVersions()` / `Class.allVersionsInRange($s, $e)` shortcuts now desugar at lower-time in `crates/pure/src/lower/member_access.rs::desugar_all_to_getall`. Routes to the corresponding `getAll*` native via the synthetic-unqualified-ptr resolution path used by the existing `Class.all()` desugaring. New `GetAllVersions` runtime native fills the last "Missing" entry in the PCT coverage audit — coverage now **100% (221/221)**, ceilings tightened to 1/2. 5 integration tests in `crates/runtime/tests/milestoning_grammar_shortcuts.rs`. GAP #12 in `docs/grammar-convergance/engine_vs_rust_analysis.md` is closed for the milestoning surface. |
