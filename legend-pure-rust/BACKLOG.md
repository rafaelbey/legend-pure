...
| Repo-boundary visibility enforcement | P1 | Java rejects cross-repo references that aren't permitted by the source file's repo's declared `dependencies`: "datamarts::datamt::domain::TestClass2 is not visible in the file /system/testFile.pure" (see `legend-pure-m2-dsl-mapping-grammar/.../TestVisibility.java`). Today the Rust pipeline parses repo descriptors into `RepoMeta { dependencies }` (`crates/core-platform-pure/src/repo.rs:70`) but no validator consults them — any FQN that resolves is accepted regardless of which repo declared it. Wire-up: (1) attach repo identity to each `SourceFile` (or to the section/element through the `SourceInfo`); (2) on every cross-repo FQN resolution (`resolve_by_path`, `resolve_function_by_path`, plus the `dsl-mapping` validator's per-body lookups for class/enum/association targets and `include` references), check the source file's repo's transitive dependencies include the resolved element's repo; (3) emit a `Visibility` diagnostic kind matching Java's message shape verbatim. Depends on the descriptor work above (need the dependency edges in-memory at compile time). Once landed, fills the **Parity #5** test gap: `TestVisibility.testClassMapping`, `testEnumMapping`, `testMappingIncludes` get Rust analogs in `crates/dsl-mapping/tests/visibility_smoke.rs` (currently absent — Mapping DSL parity audit, post-Stage 7). |
