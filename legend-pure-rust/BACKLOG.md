...
| Non-recursive structural equality | Done ✅ (equality) | `values_equal` and the object-key step are now iterative — `Vec<(Value, Value)>` work-stack plus `HashSet<(*const HeapEntry, *const HeapEntry)>` cycle guard (canonicalised pointer pair so `(A, B)` and `(B, A)` collapse). Depths are bounded by heap allocations rather than Rust stack frames, and equality-keyed self-references resolve via "assume equal on cycle" semantics matching Java's `equals`. Removes the `EQ_DEPTH` / `MAX_EQUALITY_DEPTH = 1000` stopgap that previously short-circuited deep inputs to `false` (the wrong answer). Tests: `crates/runtime/src/native/equality.rs::tests` covers 1500-deep `List<List<…>>` (past the old cap), leaf-mismatch at the same depth, and the multiplicity-coercion branches. `toRepresentation`, `toString`, `deep_copy_object`, `reactivate_value` still recurse; an audit + iterative rewrite for each lands in a future batch (separate concrete recursion sources, not blocked by the equality fix). Independent concern that surfaced during testing: `Value::Drop` on `Box<PVector<Value>>` is itself recursive — pathological-depth collections going out of scope can overflow the drop chain. Tracked separately. |
