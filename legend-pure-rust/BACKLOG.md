...
| Milestoning — Phase B-3 (deferred variants) | P2 | The first slice covers 3 of Java's 8 `MilestoningDateSourceType` variants. Remaining: **`%latest` sentinel** (variants 4/8 — needs a new `ExprKind::LatestDate` + `Value::Latest`, ~5 file touch); **AutoMap of milestoned collections through nested property chains** (variant 4); **already-explicit milestoned QP propagating its own dates to nested property access on the result**. None are blockers — the bread-and-butter `Class.getAll($d)->map(c | $c.prop)` pattern already works. |
| Milestoning — Phase B-4 (expression validators) | Partial ⚠️ | B-4.2 (missing-date) shipped alongside B-3. B-4.1 (`%latest` only in milestoning context) and B-4.3 (`%latest` forbidden in AllVersionsInRange) are deferred — they only become meaningful once `%latest` ships as a recognised expression kind. |
